...
# This function will be called conditionally based on the theme toggle.
...
# This function adds the hovering button to the bottom right corner.
...
# 1. Add the floating button (Remember to change the URL!)
add_floating_button("https://your-portfolio-url.com")

# 2. Create the theme toggle and title
col1, col2 = st.columns([4, 1])
with col1:
    # We will dynamically set the title based on the theme
    title_text = "QR Code Generator `// Light+`"
with col2:
    # The st.toggle widget returns True if it's on (Light Mode), and False if off (Dark Mode)
    is_light_mode = st.toggle("💡 Light Mode", value=False)

# 3. Apply the theme based on the toggle's state
if not is_light_mode:
    apply_dark_theme()
    title_text = "QR Code Generator `// Dark+`"

# Display the title after the theme is set
st.title(title_text)

st.write("Enter data to encode and customize your QR code in the sidebar.")

# --- Session State and Main App Logic ---
if 'qr_code_image' not in st.session_state: st.session_state.qr_code_image = None

st.sidebar.header("`Settings`")
col1, col2 = st.sidebar.columns(2)
fill_color = col1.color_picker("Fill color", "#000000" if is_light_mode else "#FFFFFF")
back_color = col2.color_picker("BG color", "#FFFFFF" if is_light_mode else "#1E1E1E")
