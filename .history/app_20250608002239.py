...
def apply_light_theme():
    ...
    light_theme_css = """
        <style>
            ...
            /* --- FIX FOR "Settings" and "Advanced" HEADERS --- */
            /* This is a more specific rule to override Streamlit's default code block style */
...
def create_clickable_title():
    ...
    title_comment = '// Dark+' if current_theme == 'dark' else '// Light+'
    
    title_html = f"""
        <style>
            .title-container {{ display: flex; align-items: baseline; gap: 12px; }}
            .theme-switcher-link {{
                font-size: 1.5rem; font-weight: 400; color: #888; text-decoration: none;
                transition: color 0.3s ease; position: relative; top: -5px;
            }}
